...
export class SensorHomeComponent implements OnInit {
  ...
  constructor(private router: Router, private dataReadService: DataReadService, private spinnerService: SpinnerService) { }
  ...
  calculateInfluenzaRatius(apiData: any): void {
    this.temperature = Number(apiData.temp);
    this.humidity = Number(apiData.hum);
    if (this.temperature < this.temperatureLevelHighRisk && this.humidity < this.humidityLevelHighRisk) {
      this.influenzaRisk = 'Muy Alta';
      this.influenzaRiskDescription = 'Ventile la habitación, y aumente la temperatura de la misma y su humedad';
      this.influenzaClass = 'extraHighRisk';
    } else if (this.temperature < this.temperatureLevelHighRisk || this.humidity < this.humidityLevelHighRisk) {
      this.influenzaRisk = 'Alta';
      this.influenzaRiskDescription = 'Ventile la habitación, y ';
      this.influenzaRiskDescription += this.temperature < this.temperatureLevelHighRisk
                                        ? 'aumente la temperatura de la habitación'
                                        : 'aumente la humedad de la habitación';
      this.influenzaClass = 'highRisk';
    } else if (this.temperature < this.temperatureLevelMediumRisk && this.humidity < this.humidityLevelMediumRisk) {
      this.influenzaRisk = 'Media';
      this.influenzaRiskDescription = 'Aumente la temperatura de la habitación o su humedad';
      this.influenzaClass = 'mediumRisk';
    } else {
      this.influenzaRisk = 'Baja';
      this.influenzaRiskDescription = 'Las condiciones de temperatura y humedad de la habitación son las adecuadas';
      this.influenzaClass = 'lowRisk';
    }
  }

  calculateParticlesRisk(apiData: any): void {
    this.pm25 = Number(apiData.pm25);
    this.pm100 = Number(apiData.pm100);
    if (this.pm25 > this.particlesLevelExtraHighRisk && this.pm100 > this.particlesLevelExtraHighRisk) {
    ...
    } else if (this.pm25 > this.particlesLevelHighRisk || this.pm100 > this.particlesLevelHighRisk) {
      this.particlesRisk = 'Alta';
      this.particlesRiskDescription = 'Aspire el polvo de la estancia';
      this.particlesClass = 'highRisk';
    } else if (this.pm25 > this.particlesLevelMediumRisk || this.pm100 > this.particlesLevelMediumRisk) {
      ...
      this.particlesRiskDescription = 'El nivel de partículas en suspensión es el adecuado';
